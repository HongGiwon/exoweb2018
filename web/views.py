...
def demo(req):
    ...
    if qid != -1:
        ...
        for question in quetions_2:
            if(question["id"].split(':::')[1] == qid):
                tmp_id = question["id"].split(':::')[0]
                tmp_q = question["text"]
                candidates2 = question["pred"]
                gold2 = question["gold"]
                break

        result['selected_question_id2'] = tmp_id
        result['selected_question_text2'] = tmp_q

        candidate_tmp2 = []
        for can2 in candidates2:
            candidate_tmp2.append(["{0}".format(can2), can2 in gold2])
        ...
        graph2 = {"nodes":[], "edges":[]}
        concept_graph2 = {"nodes":[], "edges":[]}

    with open(BASE_DIR + '/web/statics/graph2.json', 'w') as f:
        json.dump(graph2, f)
